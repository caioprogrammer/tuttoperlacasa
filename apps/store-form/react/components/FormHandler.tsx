...
import { sendGeolocations } from '../utils/helpers'
...
export const FormHandler: FC<{
  ...
  //const [geolocations, setGeolocations] = useState({ latitude: '', longitude: ''})
  ...
  const onSubmit = useCallback(
    async ({ data, methods, event }: OnSubmitParameters) => {
      ...
      const geolocations = await sendGeolocations(data)
      const hasGeolocations = props.formProps.sendGeolocations
        && props.schema.properties.latitude
        && props.schema.properties.longitude
        && geolocations

      const parsedData = parseDateTimeFieldsData({
        data: Object.assign(data, hasGeolocations
          ? { latitude: geolocations.lat, longitude: geolocations.lon }
          : { latitude: '', longitude: ''}),
  ...
  if (!props.formProps.showFormOnSubmit && submitState.success) {

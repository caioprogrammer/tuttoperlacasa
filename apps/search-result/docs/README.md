...
| `search-title.v2`         | Displays a title for the search that was done. |         
| `paged-controls` | Display paginate controls |
...
#### `paged-controls` block

| Prop name       | Type            | Description      | Default value |
| --------------- | --------------- | ---------------- | ------------- |
| `maxPageButtons` | `number` | number of page buttons to be showed | `5` |
| `showFirstPage` | `boolean` | if first page button should be showed when this page is out of range of pages limited by maxPageButtons prop | `true` |
| `showLastPage`  | `boolean` | if last page button should be showed when this page is out of range of pages limited by maxPageButtons prop  |`true` |
| `previousPageText` | `string` | text to be displayed in previous page button | "`Anterior`" |
| `nextPageText`  | `string` | text to be displayed in next page button  | `Próximo`" |
| `PreviousPageIcon`  | `block` | icon to be displayed in previous page button  | `undefined` |
| `NextPageIcon` | `block` | text to be displayed in next page button  | `undefined` |
| `NextPageIcon` | `boolean` | if will be scrolled to top after page change | `false` |
| `scrollToTop` | `enum` | Scrolls the page to the top (`auto` or `smooth`) or not (`none`) when selecting a facet. | `smooth` |

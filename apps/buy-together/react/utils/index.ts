import { InstallmentItem, InstallmentOption } from '../typings/types'
...
export const getBetterInstallments = (installmentList: InstallmentOption[]) => {
  const betterInstallment = installmentList
    ?.reduce(
      (acc, installmentPerPayment) =>
        acc.concat(installmentPerPayment.installments),
      [] as InstallmentItem[]
    )
    ?.reduce(
      (acc: InstallmentItem, installmentItem: InstallmentItem) => {
        if (installmentItem.count > acc.count) {
          return installmentItem
        }
        return acc
      },
      { value: 0, count: 0, interestRate: 0, total: 0 } as InstallmentItem
    )

  return betterInstallment
}

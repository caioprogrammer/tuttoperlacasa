...
import { useSearchPlus } from '../contexts/SearchPlus'
...
const FilterOptionTemplate = ({
  ...
  const { quantityPerScroll } = useSearchPlus()
  ...
  return (
    <div className={containerClassName} ref={filterRef}>
      ...
      <div className={`${handles.filterWrapperOptions} ${isOpen && handles.filterIsOpen}`} data-scroll={quantity >= quantityPerScroll ? 'visible' : 'hidden' } >
      <div
        ...
        aria-hidden={!isOpen}
        data-scroll={quantity >= quantityPerScroll ? 'visible' : 'hidden' }
      >
        {!hasBeenViewed ? (
          dummyElement
        ) : collapsable ? (
